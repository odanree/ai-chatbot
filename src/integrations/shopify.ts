	...
	try {
		...
		return edges.map((edge) => {
			...
			return {
				...
				handle: handle,
				url: handle
					? `${HEADLESS_STOREFRONT_URL}/products/${handle}`
					: undefined,
				price: minPrice as string,
				image: (product.featuredImage as Record<string, unknown>)?.url as
					| string
...
export async function getOrderStatus(orderId: string): Promise<ShopifyOrder> {
  ...
  `;
	...
	try {
		...
		const order = (data.data as Record<string, unknown>).order as Record<
			string,
			unknown
		>;
...
export async function getCustomerData(
  ...
  `;
	...
	try {
		...
		const customersEdges = (
			(data.data as Record<string, unknown>).customers as Record<
				string,
				unknown
			>
		).edges as Array<Record<string, Record<string, unknown>>>;
